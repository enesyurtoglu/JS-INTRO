...
//names.filter((x) => x.length === 4).forEaach((x) => connsole.log(x));

const namesWith4Chars = names.filter((x) => x.length === 4);// condition true dönerse eklemeyi yapıyor.

console.log(namesWith4Chars);

// How many names has 'a' or 'A' in it : 4  -> reduce() is preferred for this since we are counting the result i.e: .length

//const namesWithA = names.filter((name)) => {
    //return name.toLowerCase().includes('a');
//}
console.log(names.filter((name) => name.toLowerCase().includes('a')).length); // names.filter((name) => name.toLowerCase().includes('a')) bu kısım array oluşturuyor.
